...
const bCrypt = require('bcryptjs')  // Use it For hashing items like passowrd
...
}, {    // seond Object for timestamps  [Create two columns [CreatedAt & UpdatedAt] when create new user or update him]
...
userSchema.methods.generateAuthToken = async function() {   // Regular funtion to able to use this keyword
    ...
    const token = jwt.sign({ _id: user._id.toString() }, 'MySecretWord') // jwt.sign --> Generate token, takes two args [Object of What we want to token it, Any of secret word] but remember this secret to use it to verify this token to return the original data
